...
# VR_group_list = [[950253287, 849437495], [456247757]]
# VR_name_list = ['轴伊', '虚幻引擎']
VR_uid_list = [205889201, 233114659, 436344151]
VR_group_list = [[857743425], [857743425], [857743425]]
VR_name_list = ['碧蓝海事局', '碧蓝航线', 'xnxnsrz']
...
def GetDynamicStatus(uid, VRindex):
    ...
    res = requests.get('https://api.vc.bilibili.com/dynamic_svr/v1/dynamic_svr/space_history?host_uid=' + str(
        uid) + 'offset_dynamic_id=0')
    res.encoding = 'utf-8'
    res = res.text
    ...
    while last_dynamic_str != cards_data[index]['desc']['dynamic_id_str']:
        ...
        try:
            ...
            if (cards_data[index]['desc']['type'] == 64):
                content_list.append(VR_name_list[VRindex] + '发了新专栏「' + cards_data[index]['card']['title'] + '」并说： ' +
                                    cards_data[index]['card']['dynamic'])
            else:
                if (cards_data[index]['desc']['type'] == 8):
                    content_list.append(
                        VR_name_list[VRindex] + '发了新视频「' + cards_data[index]['card']['title'] + '」并说： ' +
                        cards_data[index]['card']['dynamic'])
                else:
                    if ('description' in cards_data[index]['card']['item']):
                        # 这个是带图新动态
                        content_list.append(
                            VR_name_list[VRindex] + '发了新动态： ' + cards_data[index]['card']['item']['description'])
                        # print('Fuck')
                        ...
                        if 'origin_user' in cards_data[index]['card']:
                            ...
                            content_list.append(VR_name_list[VRindex] + '转发了「' + origin_name + '」的动态并说： ' +
                                                cards_data[index]['card']['item']['content'])
                        else:
                            # 这个是不带图的自己发的动态
                            content_list.append(
                                VR_name_list[VRindex] + '发了新动态： ' + cards_data[index]['card']['item']['content'])
...
def GetLiveStatus(uid):
    res = requests.get(
        'https://api.live.bilibili.com/room/v1/Room/get_info?device=phone&;platform=ios&scale=3&build=10000&room_id=' + str(
            uid))
